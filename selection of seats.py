...
def seats_no(seats, routes):
    if seats and routes:
        try:
            conn = sqlite3.connect('seats.db')
            c = conn.cursor()
            c.execute("INSERT INTO seats (seats, routes) VALUES (?, ?)",
                      (seats, routes))
            conn.commit()
            conn.close()
            messagebox.showinfo("Success", "Booking Conformed!")
...
class BusTicketBooking:
    ...
    def confirm_booking(self):
        if self.selected_seats:
            ...
            seats_no(', '.join(seat_numbers), selected_route)
            messagebox.showinfo("Booking Confirmed", f"Route: {selected_route}\nSeats booked: {', '.join(seat_numbers)}")
